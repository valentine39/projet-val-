...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ── Tableau comparatif Pilier 5 ──
    st.markdown('<div class="sub-label">Récapitulatif des indicateurs financiers disponibles</div>', unsafe_allow_html=True)
    rows_p5 = []
    for code in fin_codes:
        meta = INDICATORS.get(code, {})
        df_p5 = all_series.get(code, pd.DataFrame())
        val, yr = get_service().wb.latest_value(df_p5)
        if val is not None:
            scaled = val / meta.get("scale", 1)
            val_str = f"{scaled:{meta.get('format', '.1f')}} {meta.get('unit_display', '')}"
        else:
            val_str, yr = "N/D", "—"
        rows_p5.append({
            "Indicateur": meta.get("label", code),
            "Valeur": val_str,
            "Année": str(yr) if yr else "—",
            "Code WB": code,
        })
    if rows_p5:
        st.dataframe(pd.DataFrame(rows_p5), use_container_width=True, hide_index=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    energy_kpi = next((k for k in kpis if k["code"] == "EG.USE.PCAP.KG.OE"), None)

    co2pc_val  = co2pc_kpi["value_display"]  if co2pc_kpi  and co2pc_kpi["value_raw"]  else "N/D"
    renew_val  = renew_kpi["value_display"]  if renew_kpi  and renew_kpi["value_raw"]  else "N/D"
    forest_val = forest_kpi["value_display"] if forest_kpi and forest_kpi["value_raw"] else "N/D"

    st.markdown(f"""
    <div class="overview-ribbon">
        <strong>Émissions CO₂</strong> : <strong>{co2pc_val} t/hab.</strong>.
        <strong>Part renouvelables</strong> dans l'électricité : <strong>{renew_val}%</strong>.
        <strong>Couverture forestière</strong> : <strong>{forest_val}% du territoire</strong>.
        Données environnementales issues de la Banque Mondiale WDI.
    </div>
    """, unsafe_allow_html=True)

    # ── KPIs ──
    st.markdown('<div class="sub-label">Indicateurs environnementaux clés</div>', unsafe_allow_html=True)
    ...
    df_energy = all_series.get("EG.USE.PCAP.KG.OE", pd.DataFrame())

    # ── Émissions CO₂ ──
    st.markdown('<div class="sub-label">Émissions de CO₂</div>', unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    ...
    with col1:
        if not df_co2pc.empty:
            ...
            no_data_placeholder("Émissions CO₂ / habitant")

    with col2:
        if not df_co2kt.empty:
            df_co2mt = df_co2kt.copy()
            df_co2mt["value"] = df_co2mt["value"] / 1000  # kt → Mt
            fig = make_bar_chart(
                df_co2mt, "EN.ATM.CO2E.KT",
                "Émissions CO₂ totales (Mt)",
                y_suffix=" Mt",
                color=COLORS["orange"],
            )
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
            st.caption("*Banque Mondiale — EN.ATM.CO2E.KT (converties en Mt)*")
        else:
            no_data_placeholder("Émissions CO₂ totales")

    # ── Énergie & forêts ──
    st.markdown('<div class="sub-label">Transition énergétique & capital naturel</div>', unsafe_allow_html=True)
    col3, col4 = st.columns(2)
    ...
    with col3:
        ...
        if not df_renew.empty:
            series_renew["EG.ELC.RNEW.ZS"] = df_renew
            labels_renew["EG.ELC.RNEW.ZS"] = "Électricité renouvelable (%)"
        if not df_energy.empty:
            series_renew["EG.USE.PCAP.KG.OE"] = df_energy
            labels_renew["EG.USE.PCAP.KG.OE"] = "Consommation énergie/hab. (kgep)"

        if series_renew:
            # Deux axes Y si les deux séries sont présentes (unités très différentes)
            if len(series_renew) == 2:
                import plotly.graph_objects as go as _go
                fig_dual = go.Figure()
                color_list = list(COLORS.values())
                fig_dual.add_trace(go.Scatter(
                    x=df_renew.dropna(subset=["value"])["year"],
                    y=df_renew.dropna(subset=["value"])["value"],
                    name="Électricité renouvelable (%)",
                    line=dict(color=COLORS["green"], width=2),
                    yaxis="y1",
                ))
                fig_dual.add_trace(go.Scatter(
                    x=df_energy.dropna(subset=["value"])["year"],
                    y=df_energy.dropna(subset=["value"])["value"],
                    name="Consommation énergie/hab. (kgep)",
                ...
                fig_dual.update_layout(
                    ...
                    margin=dict(l=10, r=50, t=20, b=65),
                    height=280,
                    hovermode="x unified",
                    xaxis=dict(gridcolor="#e8e6e2"),
                    yaxis=dict(gridcolor="#e8e6e2", ticksuffix="%", title="% renouvelable"),
                    yaxis2=dict(overlaying="y", side="right", showgrid=False,
                                ticksuffix=" kgep", title="kgep/hab."),
                )
                st.plotly_chart(fig_dual, use_container_width=True, config={"displayModeBar": False})
                st.caption("*Banque Mondiale — EG.ELC.RNEW.ZS · EG.USE.PCAP.KG.OE*")
            else:
                fig = make_line_chart(series_renew, labels_renew, y_suffix="%")
                st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
                st.caption("*Banque Mondiale — EG.ELC.RNEW.ZS*")
        else:
            no_data_placeholder("Énergies renouvelables / consommation énergétique")

    with col4:
        if not df_forest.empty:
            fig = make_line_chart(
                {"AG.LND.FRST.ZS": df_forest},
                {"AG.LND.FRST.ZS": "Couverture forestière (% territoire)"},
                fill=True,
                y_suffix="%",
            )
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    # ── Note contextuelle sur les données climatiques ──
    st.markdown('<div class="sub-label">Note méthodologique</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="overview-ribbon">
        <strong>Sources disponibles (Phase 1) :</strong> La Banque Mondiale WDI fournit des données
        sur les émissions de CO₂, la part des renouvelables et la couverture forestière.
        <br><br>
        <strong>Sources prévues (Phase 2) :</strong> Des indicateurs de vulnérabilité climatique avancés
        (ND-GAIN Index, Germanwatch CRI, indices de risque physique IPCC) seront intégrés dans une 
        prochaine version via des sources spécialisées. Les données sur les catastrophes naturelles 
        (EM-DAT), la biodiversité (UICN) et les politiques climatiques (NDC Registry, IRENA) 
        compléteront ce pilier.
    </div>
    """, unsafe_allow_html=True)

    # ── Tableau récap Pilier 6 ──
    st.markdown('<div class="sub-label">Récapitulatif des indicateurs environnementaux disponibles</div>', unsafe_allow_html=True)
    rows_p6 = []
    for code in clim_codes + ["EN.ATM.CO2E.KT"]:
        meta = INDICATORS.get(code, {})
        if not meta:
            continue
        df_p6 = all_series.get(code, pd.DataFrame())
        val, yr = get_service().wb.latest_value(df_p6)
        if val is not None:
            scaled = val / meta.get("scale", 1)
            val_str = f"{scaled:{meta.get('format', '.2f')}} {meta.get('unit_display', '')}"
        else:
            val_str, yr = "N/D", "—"
        rows_p6.append({
            "Indicateur": meta.get("label", code),
    ...
    if rows_p6:
        st.dataframe(pd.DataFrame(rows_p6), use_container_width=True, hide_index=True)



st.markdown("---")
st.markdown('<div class="sub-label">📋 Tableau récapitulatif sourcé — tous les indicateurs</div>', unsafe_allow_html=True)

# Rendre les liens cliquables avec st.data_editor ou HTML
if not summary_df.empty:
    # Afficher colonnes sans le lien d'abord, puis ajouter colonne lien HTML
    display_df = summary_df[["Indicateur", "Valeur", "Année", "Unité", "Source"]].copy()

    # Lien cliquable via markdown
    link_col = summary_df["Lien"].apply(
        lambda url: f"[🔗 Voir]({url})" if url else "—"
    )
    display_df["Lien source"] = link_col
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Lien source": st.column_config.LinkColumn("Lien source", display_text="🔗 Voir"),
        },
        height=min(50 + len(display_df) * 38, 600),
    )
else:
